...
function validate_login_form(form, data) {
  if (data.login_username == "") {
    ...
    addFormError(form["login_username"], 'The username is invalid');
    return false; // stop the script if validation is triggered
  }

  if (data.login_password == "") {
    // if password variable is empty
    addFormError(form["login_password"], 'The password is invalid');
    return false; // stop the script if validation is triggered
  }
  $.ajax({
  ...
  if (validality == "fail"){
    addFormError(form["login_username"], 'The username does not exist or password incorrect');
    return false;
  }
  ...
  $.cookie('username',data.login_username);
...
function validate_word(form,data){
  console.log("???");
  if(data.input_word.length!=5){
    addFormError(form["input_word"],"You need to enter a 5-letter word with no repeating letters");
    return false;
  }
  ...
  //Start game and put the word into database
...
form_handler("#new_game",validate_word,null, null, null, null, null, null);
